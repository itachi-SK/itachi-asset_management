...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $employeeSql = "SELECT emp_name, emp_designation, emp_email, emp_phone, site_name, username FROM employee WHERE id = $employeeId";
    ...
    // Get username from the employee details
    $employeeUsername = $employeeRow['username'];

    // Insert data into allocated_asset table
    $insertSql = "INSERT INTO allocated_asset (username, emp_name, emp_designation, emp_email, emp_phone, site_name, allocation_date, reason_asset, asset_type, asset_sn, asset_model, asset_accessories, asset_warranty)
    VALUES ('$employeeUsername', '{$employeeRow['emp_name']}', '{$employeeRow['emp_designation']}', '{$employeeRow['emp_email']}', '{$employeeRow['emp_phone']}', '{$employeeRow['site_name']}', '$allocationDate', '$reasonAsset', '{$assetRow['asset_type']}', '{$assetRow['asset_sn']}', '{$assetRow['asset_model']}', '{$assetRow['asset_accessories']}', '{$assetRow['asset_warranty']}')";
...
<body>
    ...
    <div id="allocateTableContainer" class="card-container1">
        <div class="col-lg-6 m-auto">
            <form method="POST" action="">
                <div class="card">
                    <div class="card-header bg-warning">
                        <h1 class="text-white text-center">Allocation Form</h1>
                    </div>
                    ...
                    <select name="employee_id">
                        ...
                        while ($employeeRow = $employeeResult->fetch_assoc()) {
                            echo "<option value='" . $employeeRow['id'] . "'>" . $employeeRow['emp_name'] . "</option>";
                        }
                        ?>
                    </select>
                    ...
                    <select name="asset_id">
                        ...
                        while ($assetRow = $assetResult->fetch_assoc()) {
                            echo "<option value='" . $assetRow['id'] . "'>" . $assetRow['asset_type'] . "</option>";
                        }
                        ?>
                    </select>
